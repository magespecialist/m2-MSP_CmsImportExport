...
class Content implements ContentInterface
{
    ...
    protected $_cmsMode;
    protected $_mediaMode;
    protected $_storesMap;
    ...
    public function __construct(
        ...
        $this->_cmsMode = ContentInterface::CMS_MODE_UPDATE;
        $this->_mediaMode = ContentInterface::MEDIA_MODE_UPDATE;

        $this->_storesMap = [];
        $stores = $this->storeRepositoryInterface->getList();
        foreach ($stores as $store) {
            $this->_storesMap[$store->getCode()] = $store->getCode();
        }
    }
    ...
    protected function _mapStores($storeCodes)
    {
        ...
        foreach ($storeCodes as $storeCode) {
            foreach ($this->_storesMap as $to => $from) {
                if ($storeCode == $from) {
                    $return[] = $to;
                }
            }
    ...
    public function importPageFromArray(array $pageData)
    {
        ...
        if ($pageId) {
            ...
            if ($this->_cmsMode == ContentInterface::CMS_MODE_SKIP) {
                return false;
            }
        }
    ...
    public function importFromArray(array $payload, $archivePath = null)
    {
        ...
        // Import media
        if ($archivePath && ($count > 0) && ($this->_mediaMode != ContentInterface::MEDIA_MODE_NONE)) {
            foreach ($payload['media'] as $mediaFile) {
                ...
                if ($this->file->fileExists($sourceFile, true)) {
                    ...
                    if (
                        ...
                        ($this->_mediaMode == ContentInterface::MEDIA_MODE_SKIP)
                    ) {
                        continue;
                    }
